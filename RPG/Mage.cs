...
namespace RPG
{
    sealed class Mage : Hero
    {
        ...
        {
            int gain = 3 * (int)Math.Round(1.45 * level, 0);
        ...
        {
            return "power";//deals extra damage at cost of accuracy
        }
        ...
        public override string gainXP(int xp)
        {
            string output = "";
            base.gainXP(xp);
            while (this.xp >= ((level * level / 3.75) * 100))
            {
                this.xp -= (int)(Math.Round(((level * level / 3.75) * 100), 0));
                output += LevelUp();
            }
            return output;
        }
        ...
        public override string exp()
        {
            ...
            maxxp = (int)Math.Round(((level * level / 3.75) * 100), 0);
                return base.exp();
