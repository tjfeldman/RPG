...
namespace RPG
{
    sealed class Scout :Hero
    {
        public Scout(World world)
        {
            title = "Scout";
            setStats(world);
        }
       ...
       public override string LevelUp()
        ...
        {
            return "scout";//critials do 3x damage instead.
        }
        ...
        public override string gainXP(int xp)
        {
            string output = "";
            base.gainXP(xp);
            while (this.xp >= ((level * level / 4.8) * 100))
            {
                this.xp -= (int)(Math.Round(((level * level / 4.8) * 100), 0));
                output += LevelUp();
            }
            return output;
        }
