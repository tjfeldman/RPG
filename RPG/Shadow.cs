...
namespace RPG
{
    sealed class Shadow:Boss
    {
        ...
        public override void getStats(int level)
        {
            level++;
            Random r = Program.r;
            atk = (int)(Math.Round((r.Next(20, 41) + (level * 6.0)), 0));
            def = (int)(Math.Round((r.Next(5, 16) + (level * 6.0)), 0));
            hp = (int)(Math.Round((r.Next(50, 101) + (level * 15.5)), 0));
            spd = (int)(Math.Round((r.Next(20, 31) + (level * 6.0)), 0));
            maxhp = hp;
            this.level = level;
            base.getStats(level);
        }
        ...
        {
            int xp = (int)(Math.Round(((300 * level) / 5.25), 0));
            return xp;
        }
