...
namespace RPG
{
    sealed class Warrior : Hero
    {
        ...
        {
            int gain = 2 * (int)Math.Round(1.25 * level, 0);
        ...
        {
            return "guard";//chance to block incoming attacks
        }
        ...
        public override string gainXP(int xp)
        {
            string output = "";
            base.gainXP(xp);
            while (this.xp >= ((level * level / 3.5) * 100))
            {
                this.xp -= (int)(Math.Round(((level * level / 3.5) * 100), 0));
               output += LevelUp();
            }
            return output;
        }
        ...
        public override string exp()
        {
            ...
            maxxp = (int)Math.Round(((level * level / 3.5) * 100), 0);
                return base.exp();
